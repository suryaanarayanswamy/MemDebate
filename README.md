...
memdebate/
│
├── memdebate.py              # Core implementation
│   ├── MemDebate             # Main class — debate gate + eviction
│   ├── StoreAllAgent         # Baseline 1
│   └── PeriodicSummarizationAgent  # Baseline 2
│
├── MemDebate.ipynb           # Full walkthrough notebook
│   ├── Data structures
│   ├── Core implementation
│   ├── Live demo (mock mode)
│   ├── LoCoMo evaluation
...
This runs in **mock mode** — a heuristic confidence scorer simulates the debate gate so you can see the full pipeline without an OpenAI key.

### 3. Run with real GPT-4o agents

```bash
export OPENAI_API_KEY=sk-...
python memdebate.py   # set use_mock=False inside the script
```
